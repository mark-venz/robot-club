...
EmRGBLed::EmRGBLed(uint8_t port, uint8_t led_num)
{
...
void EmRGBLed::setpin(uint8_t port)
{
  pinMask   = digitalPinToBitMask(port);
  ws2812_port = portOutputRegister(digitalPinToPort(port) );
  pinMode(port, OUTPUT);
...
void EmRGBLed::setNumber(uint8_t num_leds)
{
  count_led = num_leds;
  ...
  for(int8_t i = 0; i < count_led * 3; i++)
    {
      pixels[i] = 0;
    }
}

bool EmRGBLed::setColorAt(uint8_t index, uint8_t red, uint8_t green, uint8_t blue)
{
  if(index < count_led)
  {
    uint8_t tmp = index * 3;
...
bool EmRGBLed::setColorAll(uint8_t red, uint8_t green, uint8_t blue)
{
...
/* ******************************************************************************
 * This is the tricky bit that needs to be stolen ^_^
 *
 * For an explaination as to how the LED modules work.
 * See: https://learn.sparkfun.com/tutorials/ws2812-breakout-hookup-guide/ws2812-overview
 *
 * its getting the timing right.
 */
/*
 ...
 *   None
 */
void EmRGBLed::rgbled_sendarray_mask(uint8_t *data, uint16_t datlen, uint8_t maskhi, uint8_t *port)
{
